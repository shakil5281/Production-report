...
export function MobileResponsivePagination({
  pageIndex,
  pageSize,
  pageCount,
  totalRows,
  ...
  maxVisiblePages = 5,
  className = ''
}: MobileResponsivePaginationProps) {
  const isMobile = useIsMobile();
  
  // Calculate current page (TanStack uses 0-based indexing)
  ...
  const endRow = Math.min(pageIndex * pageSize + pageSize, filteredRows);
        ...
        {showRowCount && (
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
            <div className="text-sm text-muted-foreground">
              {filteredRows > 0 ? (
                <>
                  Showing {startRow} to {endRow} of {filteredRows} entries
                  {totalRows !== filteredRows && ` (filtered from ${totalRows} total)`}
                </>
              ) : (
                'No entries to show'
              )}
